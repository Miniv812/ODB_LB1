# Лабораторна робота 2: Перетворення ER-діаграми на схему PostgreSQL

У ході цієї лабораторної роботи я перетворив мою ER-модель на фактичну реляційну схему

Подивитися мої SQL-скрипти з операторами Create Table можна у файлі CREATE_TABLE

Перевіримо на виконання створення таблицю шляхом виконання оператора INSERT:

Перевірка табилці User:
<img width="1074" height="545" alt="{35AA4D66-D080-4988-87EE-61E105DCDAC2}" src="https://github.com/user-attachments/assets/b7d7dc4f-00f8-4d4e-81fb-583e1eece275" />

Перевірка табилці Guild:
<img width="1137" height="441" alt="{7E8A9549-6E1C-4F94-AC79-928510862B66}" src="https://github.com/user-attachments/assets/5974fdae-671e-4383-b795-55362944c913" />

Перевірка табилці Player:
<img width="1163" height="843" alt="{4FF4FB07-C5C6-48D0-B428-140D7559F2FC}" src="https://github.com/user-attachments/assets/9540d1a9-3825-479a-9b9e-deff2db9d248" />

Перевірка табилці Item:
<img width="1292" height="843" alt="{940F02D9-C35F-40DC-991B-DD00D3DBDE49}" src="https://github.com/user-attachments/assets/cff8320f-ede2-48e7-a989-974f1523091a" />

Перевірка табилці player_item:

<img width="660" height="879" alt="{145C9C47-6612-41AD-8EEB-0ACF1911B0EC}" src="https://github.com/user-attachments/assets/996d93ac-aeb1-4e30-bbba-3ad0eed885f7" />

Перевірка табилці location:
<img width="1084" height="665" alt="{A5C6C2A5-F572-4FD7-868C-DE390E10010A}" src="https://github.com/user-attachments/assets/f1ed5a86-852d-40ad-9412-94935379b573" />

Перевірка табилці creature:

<img width="1422" height="842" alt="{97EDDD44-B92F-4ACD-AC8B-412E7CC00AD3}" src="https://github.com/user-attachments/assets/29dad549-e207-42c1-bd63-1bc7afab7353" />



Отже після всіх тестів я перевір на роботоспособність мою схему.

У ході виконня лабораторної роботи я більше дізнався про SQL-скрипти, а саме оператори Create Table та INSERT
